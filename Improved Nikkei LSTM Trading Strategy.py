...
SEQUENCE_LENGTH = 100
LOOKAHEAD_PERIOD = 10
SCALING_FACTOR = 100  # 調整済み
THRESHOLD = 0.05
MAX_POSITION = 2.0
TRANSACTION_COST = 0.001
ROLLING_WINDOW = 126 * 66  # 6ヶ月分
...
def engineer_features(df):
    ...
    df['Volatility'] = df['Returns'].rolling(66).std()  # 1日のボラティリティ
...
def rolling_scale_data(df, features, target_col, window):
    ...
    for i in range(len(df)):
        ...
        if len(window_data) > 1:
            ...
            scaled_features[i] = feature_data[i]
    
    return scaled_features, target_data, feature_scaler, None
...
def main():
    ...
    feature_data, target_data, feature_scaler, _ = rolling_scale_data(
    ...
    print("モデルを構築中...")
    model = Sequential([
        LSTM(128, return_sequences=True, input_shape=(SEQUENCE_LENGTH, len(FEATURES)), kernel_regularizer=l2(0.01)),
        Dropout(0.3),
    ...
    # 予測値をスケール調整
    future_return_std = data['Future_Return'].std()
    if data['Predicted_Return'].std() != 0:
        data['Predicted_Return'] = data['Predicted_Return'] * (future_return_std / data['Predicted_Return'].std())
    ...
    dynamic_threshold = max(THRESHOLD * pred_std / future_return_std, 0.2)  # より高い閾値
    ...
    data['Position_Size'] = np.where(data['Volatility'] > 0.01, 0, data['Position_Size'])  # 高ボラティリティ時に取引停止
