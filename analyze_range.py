...
# データディレクトリのパス
data_dir = './data_utf8/2024/11'  # 必要に応じて変更
    ...
    for file_name in sorted(files):  # 日付順に処理するためソート
        if file_name.endswith('.csv'):
            ...
            try:
                ...
                # 必要な行と列を列を抽出。行は0-78行目、列はP:U列を抽出
                ...
                print(df.head(10))  # デバッグ用出力
                print(df.tail(10))  # デバッグ用出力
                
                # 有効なデータのみを対象に合計とカウントを更新
                # valid_ranges = df['range'].dropna()  # NaNを除外
                # print(valid_ranges.head(10))  # デバッグ用出力
                # print(valid_ranges.tail(10))  # デバッグ用出力
